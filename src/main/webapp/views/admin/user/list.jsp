					...
					<div class="table-responsive category-table">
						<div>
							<c:if test="${!empty success}">
								<div class="alert alert-primary" style="background: green"
									role="alert">
									<strong>${success}</strong>
								</div>
							</c:if>
							<c:if test="${!empty faild}">
								<div class="alert alert-dangery" style="background: red"
									role="alert">
									<strong>${faild}</strong>
								</div>
							</c:if>
							<table class="table all-package theme-table" id="table_id">
								<thead>
									<tr>
										<th style="color: #ffa53b">User Name</th>
										<th style="color: #ffa53b">Email</th>
										<th style="color: #ffa53b">Phone</th>
										<th style="color: #ffa53b">Address</th>
										<th style="color: #ffa53b">Role</th>
								...
								<tbody>
									<c:forEach items="${list}" var="u" varStatus="loop">
										<tr>
											...
											<td><strong>${u.telephone}</strong></td>
											<td><strong>${u.address}</strong></td>
											<td><strong> 
												<c:forEach items="${u.userRoles}"
														var="userRole" varStatus="roleLoop">
										                ${userRole.role.name}
										                <c:if test="${!roleLoop.last}">, </c:if>
												</c:forEach>
											</strong></td>
											<td>
												<ul>
													...
													<li><a href="user/edit/${u.id}"> <i
															class="ri-pencil-line"></i>
													</a></li>

													<li><a href="user/delete/${u.id}"> <i
